        ...
        <div class="main-content">
            ...
            <div class="container mt-3">
                ...
                <div class="form-container">
                    <form action="{{ route('tasks.assign.update', $task->id) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <div class="form-group">
                            <label>Task Title</label>
                            <p class="form-control-plaintext font-weight-bold">{{ $task->task_title }}</p>
                        </div>

                        <div class="form-group">
                            <label>Currently Assigned To</label>
                            <p class="form-control-plaintext">{{ $task->assignee->user_name ?? 'Not Assigned' }}</p>
                        </div>

                        <div class="form-group">
                            <label>Select New User</label>
                            <select name="task_assigned_to" class="form-control" required>
                                <option value="">Select User</option>
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ $task->task_assigned_to == $user->id ? 'selected' : '' }}>
                                        {{ $user->user_name }}
                                    </option>
                                @endforeach
                            </select>
